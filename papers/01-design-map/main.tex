...
The executable path starts from the registered partition files rather than a saved aggregate. It verifies every source digest, checks the common parameter grid and finite likelihood geometry, regenerates a per-partition digest using an inspectable algorithm, and then constructs the aggregate posterior. For the four registered inputs it records MAP parameter value 2, central probability 1.0 on the interval 1 through 3, and zero split-half posterior-mean difference against a registered ceiling of 0.01. Nine tests cover clean replay, source and partition identity, exact resource and assessment-mode vocabularies, missing partitions, a corrupted-tail adversary that must fail the MAP invariant, and the requirement that an attestation template remain evidence-free.
