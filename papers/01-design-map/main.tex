...
\author{Codex (OpenAI), on behalf of R. O'Shaughnessy}
\date{24 August 2026}
...
\begin{description}
  ...
  \item[Bounded recomputation.] A smaller input partition or lower-cost configuration is rerun with named checks. Any scaling, convergence, or representativeness conclusion requires corresponding tests and cannot be inherited from the mode label.
...
Machine checks and the agent-assisted provenance review are recorded separately from human scientific review. R. O'Shaughnessy supplied blanket human approval of the prepared dossier for exact commit \texttt{2495989}; Codex recorded the six accept dispositions on the human's behalf without inventing a separate itemized rationale. The accepted conclusion is bounded computational repeatability of the registered path and satisfaction of one necessary invariant. It does not warrant independent replication, calibration revalidation, parameter estimation, astrophysical interpretation, or broader scientific validity.

The source-bounded dossier makes the accepted scope inspectable. The discovery paper supports four pieces of context: a 35--350~Hz visualization (with additional line-rejection filtering), an approximately 0.2-second signal, the observed 6.9~ms delay within the 10~ms intersite propagation time, and an orientation-motivated inversion for visual alignment~\citep{abbott-gw150914-discovery-2016}. A later collaboration guide demonstrates that correlation results are sensitive to bandpass and window choices and labels its own whitened, windowed, eighth-order 35--350~Hz treatment as visualization-only~\citep{abbott-lvc-data-guide-2020}. GWOSC defines the public event-time quality and injection bits, while the detector-characterization study shows why those bits do not replace the deeper background, environmental, and instrumental checks used in the scientific assessment~\citep{gwosc-technical-details-2026,gwosc-o1-data-2018,abbott-gw150914-noise-2016}. The completed disposition accepts the packet's exact unwhitened and unnotched filter path, 820-sample window, maximum-absolute-correlation convention, 0.30 floor, and boundary declaration only for the candidate conclusion. The original blank form remains as the reusable template.
...
\begin{evidencegate}[Remaining boundaries for Case A]
Qualified human GW review is recorded for exact commit \texttt{2495989}, but the approval is not an independent replication or release authorization. The replay and sign-off do not supply independent implementation, calibration, institutional custody, or input data. The packet remains unarchived, repository licensing is unresolved, and any stronger relying domain may require additional independence or itemized review rationale.
\end{evidencegate}

\section{Worked case B: HPC, large storage, or restricted data}
\label{sec:hpc}

The attached second fixture is a sanitized, synthetic partitioned-inference downselect, not a production or collaboration-internal workload. Its declared full-scale scenario is an illustrative resource shape---8192 partitions, 16 GPUs, 1800 GPU-hours, 6~TB of scratch space, 30~TB retained storage, HTCondor scheduling, restricted access, operator support, and domain judgment---rather than a measured claim about any named scientific system. The locally executed path uses four exact likelihood partitions, a standard Python interpreter, one CPU process, no scheduler, and no restricted material.

The executable path starts from the registered partition files rather than a saved aggregate. It verifies every source digest, checks the common parameter grid and finite likelihood geometry, regenerates a per-partition digest using an inspectable algorithm, and then constructs the aggregate posterior. For the four registered inputs it records MAP parameter value 2, central probability at least 0.99 on the interval 1 through 3, and split-half posterior-mean difference at most 0.01. The serialized values 1.0 and 0.0 are rounded to 12 decimal places, not exact mathematical identities. Eleven tests cover clean replay, source and partition identity, exact resource and assessment-mode vocabularies, packet-index completeness and uniqueness, rejection of coherently recorded failed invariants, missing partitions, a corrupted-tail adversary that must fail the MAP invariant, and the requirement that an attestation template remain evidence-free.

The record keeps all six assessment modes distinct. Bounded recomputation, digest audit, and frozen-output inspection are marked performed on this synthetic fixture. Full independent execution and an independent evidence path are marked not performed; restricted-platform attestation is a template only. The strongest warranted conclusion is therefore limited to sampled lineage and digest-generation behavior plus the bounded aggregate invariants. It excludes representativeness, full-scale convergence, scheduler or GPU behavior, production storage integrity, attestor honesty, independent replication, and scientific validity.

A separate fresh-context reviewer inspected exact commit \texttt{a75d671} from a detached same-host clone and returned a hold rather than a pass. Coherently recorded failed invariants could pass standalone replay, and repository validation could accept an omitted packet-index entry. The superseding implementation requires passing invariants during replay and exact unique index coverage; it also narrows the bounded-recomputation definition, qualifies rounded values, and removes unsupported resource language. A second detached inspection of exact correction commit \texttt{5f26da1} passed all eleven checks and made coherent-failure, index-omission, and duplicate-index probes fail. That clearance is limited to packet robustness. Checkout custody and executor context were partly separated; implementation, inputs, libraries, host, institution, method, and scientific judgment were not independent.

A source-bounded mapping now compares this deliberately small fixture with the public RIFT review surface. RIFT's published and documented structure includes an initial intrinsic grid, parallel ILE marginalized-likelihood evaluations, likelihood consolidation, CIP fitting and posterior sampling, iterative feedback and convergence tests, HTCondor submit files and a DAG, iteration logs and posterior products, and GPU-accelerated paths~\citep{lange-rift-2018,wysocki-rift-gpu-2019,rift-docs-overview-2026,rift-docs-pipeline-2026}. The fixture's input arrays, regenerated digests, fixed-grid posterior, and invariants are only narrow analogies to those layers. It does not execute RIFT, ILE, CIP, a DAG, or a GPU; it contains no strain, PSD, calibration, waveform, or production convergence evidence. The mapping changes no assessment-mode status and does not source the illustrative 8192-partition, GPU-hour, or storage values.
...
\centering
\small
\caption{Assessment modes exposed by the two cases. The HPC downselect has performed only bounded recomputation, digest audit, and frozen-output inspection on synthetic inputs; the other HPC rows remain unperformed paths or templates.}
\label{tab:modes}
...
\begin{evidencegate}[Remaining evidence gate for Case B]
The sanitized packet demonstrates that a downselect can begin at registered partitions, regenerate digests, preserve a ten-axis distinction between local and illustrative full-scale resources, and prevent three performed modes from being inflated into six. A bounded fresh-context inspection has tested packet robustness and preserved its own initial hold and remediation. A public RIFT documentary mapping now identifies the major scientific and operational gaps without claiming execution or representativeness. The next gate is a public RIFT execution under separately recorded custody, or qualified domain inspection plus measured resource and lineage evidence; a genuine restricted-platform attestation or independent evidence path remains a separate route. Human production-domain sign-off remains absent.
\end{evidencegate}
