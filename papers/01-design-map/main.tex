...
Consider a public-data analysis with three claims: a fitted slope is positive over a declared range; a model residual passes a prespecified diagnostic; and a sensitivity result is stable to one alternate preprocessing choice. The entire analysis runs on a contemporary laptop, requires no specialty scheduler, consumes less than 10~GB, and completes within one hour. These numbers are illustrative requirements for the case packet, not measured results.

The release identifies the downloaded raw files by digest and archive locator. A preprocessing activity produces an intermediate table; a fitting activity produces parameters and diagnostics; a rendering activity produces the table and figures. Each transition records code revision, environment lock, configuration, seed where applicable, commands, input and output digests, and expected tolerances. Claim~1 points to the fit and diagnostic evidence, not merely the figure. Claim~2 points to the residual test and the prespecified decision rule. Claim~3 points to both preprocessing branches and their comparison.

The validation package includes ordinary tests and scientific invariants. A unit test checks parsing. An integration test checks that a clean execution reaches the expected outputs. A scientific invariant checks that filtered counts and units are consistent across stages. A negative test corrupts an input header or uses an invalid unit and must fail before fitting. Another adversarial check replaces an upstream intermediate table with a same-shaped but wrong file; the workflow must detect the digest mismatch instead of happily regenerating the figure.

The review is split into at least two scopes. A provenance/software reviewer checks identities, environment reconstruction, workflow behavior, and negative tests. A scientific reviewer checks the claim definitions, diagnostics, assumptions, and interpretation. Their reports remain distinct even if one person is qualified for both. A reviewer-controlled full execution can warrant computational repeatability and, when the reviewer independently reconstructs the released path, the bounded computational-reproducibility conclusion. It cannot warrant independent replication or scientific validity without the corresponding evidence.

Freshness is a lifecycle obligation. The release declares when dependencies, external data locators, and executable tests will be checked. If a later operating-system or dependency update breaks the run, the record appends a failed-reproduction event with logs and affected scope. The reviewed release remains recoverable. A remediated release receives a new immutable identifier and a supersession relation. This avoids the fiction that a badge, once earned, is permanently current.

\begin{evidencegate}[First unmet artifact gate for Case A]
The executable \Rzero case packet has not yet been attached to this manuscript. Promotion from design walkthrough to implementation observation requires a frozen claim manifest, data inventory, digests, environment lock, tests including the corrupted-input case, an execution record, reviewer reports, and a simulated later-failure/correction event.
\end{evidencegate}
