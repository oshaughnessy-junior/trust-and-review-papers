...
Run `python3 run.py` (Python standard library only). The script regenerates `results/economics/allocation.csv`, `queues.csv`, and `results.json` under the dossier, recording configuration, seeds, Python version, script/data hashes, per-seed outcomes, summaries and descriptive Monte Carlo uncertainty. This is a small numerical exercise of the economic paper's declared models, not a calibrated test of MCRP or human behavior.
...
Numerical implementation after adversarial review uses log shadow prices, explicit shape and finite-value checks, an exact single-productive-lot path, and leaves unproductive budget unused for a flat objective. Extreme finite floating-point ranges can still raise an explicit ArithmeticError/ValueError rather than produce a false allocation. The tested synthetic parameter range is the declared sweep; this is not a general production optimizer. Regressions include the originally failing 100-hour single-lot example and zero-productivity controls.
