...
OUT=ROOT.parents[1]/'results'/'economics'


def water_fill(A, a, H, lower=None):
    if not A or len(A)!=len(a) or (lower is not None and len(lower)!=len(A)):
    ...
    if any(isinstance(v,bool) or not isinstance(v,(float,int)) or not math.isfinite(v) or v<0
           for v in [*A,*a,H,*lower]):
        raise ValueError('finite nonnegative model inputs required')
    if sum(lower)>H:
        raise ValueError('infeasible lower bounds')
    if sum(lower)==H:
        return lower
    logs=[math.log(x)+math.log(y) if x>0 and y>0 else None for x,y in zip(A,a)]
    if all(v is None for v in logs):
        return lower  # flat objective: unused resources are explicitly permitted
    active=[i for i,v in enumerate(logs) if v is not None]
    if len(active)==1:
        result=lower[:]
        result[active[0]]+=H-sum(lower)
        return result
    offset=max(v for v in logs if v is not None)
    logs=[None if v is None else v-offset for v in logs]
    def allocation(log_eta):
        return [l if v is None else max(l,(v-log_eta)/y) for v,y,l in zip(logs,a,lower)]
    hi=max(v-y*l for v,y,l in zip(logs,a,lower) if v is not None)
    span=1.0
    lo=hi-span
    while sum(allocation(lo))<H:
        span*=2
        lo=hi-span
        if not math.isfinite(lo):
            raise ValueError('budget exceeds representable shadow-price range')
    for _ in range(200):
        mid=(lo+hi)/2
        if sum(allocation(mid))>H: lo=mid
        else: hi=mid
    vals=allocation(hi)
    if abs(sum(vals)-H)>1e-9*max(H,1e-12):
        raise ArithmeticError('allocation did not resolve the budget at requested tolerance')
    return vals


def benefit(A,a,h):
    if len(A)!=len(a) or len(a)!=len(h):
        raise ValueError('benefit vectors must have equal length')
